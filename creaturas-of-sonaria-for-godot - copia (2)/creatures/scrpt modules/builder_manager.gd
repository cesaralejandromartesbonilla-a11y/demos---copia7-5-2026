	...
	if is_build_mode:
		...
		if event is InputEventMouseButton and event.pressed:
			if event.button_index == MOUSE_BUTTON_LEFT:
				...
				if result and result.collider.is_in_group("poste_electrico"):
...
func _try_connect_wire() -> void:
	...
	if result:
		var target = result.collider
		# Comprobamos si es un poste independiente o un conector integrado en una máquina
		if target.is_in_group("poste_electrico") and target.has_method("connect_to_node"):
			if first_pole_selected == null:
				...
				if target != first_pole_selected:
					...
					print("No puedes conectar un poste a sí mismo.")
...
func _update_wiring_hover() -> void:
	...
	var space_state = get_viewport().world_3d.direct_space_state
	var query = PhysicsRayQueryParameters3D.create(ray_origin, ray_end)
	# Ojo: Asegúrate de que el collision_mask aquí pueda detectar el poste eléctrico (ej: query.collision_mask = 1)
	var result = space_state.intersect_ray(query)
	
	if result:
		var target = result.collider
		if target.is_in_group("poste_electrico"):
