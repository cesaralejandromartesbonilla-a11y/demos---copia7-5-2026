...
# 0 = Ninguno, 1 = Opcional, 2 = Requerido
...
func _process(delta: float) -> void:
	# 1. Si está apagada del botón, ignoramos todo
	if not is_machine_enabled:
		if power_receiver: power_receiver.turn_machine_off()
		_change_state("OFF")
	...
	var can_run = true
	var error_state = "IDLE" # Estado cuando está encendida pero sin hacer nada
	var has_power_for_lights = false

	# 2. LÓGICA DE ELECTRICIDAD (Consume solo por estar el botón en ON)
	if power_mode != 0 and power_receiver:
		...
		if power_ratio < 1.0: 
			if power_mode == 2:
				...
				# ¡MAGIA!: NO apagamos el power_receiver aquí. 
				# Sigue exigiendo energía para no causar la epilepsia.
		else:
	...
	# 4. LÓGICA DE COMBUSTIBLE (Quema solo por estar el botón en ON)
...
func _check_for_recipes() -> void:
	for r in recipes:
		# Comprobamos ingredientes y si hay espacio en la salida (o si no hay salida)
...
func _continue_processing(delta: float) -> void:
	...
	if progress >= current_recipe.craft_time:
		...
		# Si tiene inventario de salida, lo guarda
		if output_inv and output_inv.add_item(result):
			status_changed.emit("Producto guardado en salida")
		else:
			# Si NO tiene inventario de salida, o está lleno, pide expulsarlo al mundo
...
# Esta función se sigue usando solo para las mesas manuales (con UI)
