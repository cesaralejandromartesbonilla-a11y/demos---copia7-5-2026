...
func setup(_processor: ProcessorComponent, _player_hands: Node, _base_scene: PackedScene, _name: String) -> void:
	...
	vbox_derecha.move_child(lights_btn, 1) # Lo colocamos en la posición 1 (justo debajo del botón maestro)
	vbox_derecha.move_child(power_btn, 0) # Lo movemos a la primera posición de la lista
