...
func _input(event):
	if Input.is_action_just_pressed("press_r"): ragdoll_mode = bool(1-int(ragdoll_mode)) # toggle ragdoll mode

	active_arm_left = Input.is_action_pressed("press_crlickL")# activate left arm with mouse left click
	active_arm_right = Input.is_action_pressed("press_crlickR")# activate right arm with mouse right click
...
func _physics_process(delta):
	...
	if not ragdoll_mode:# if not in ragdoll mode
		...
		#jump
		if Input.is_action_pressed("press_espace"):
